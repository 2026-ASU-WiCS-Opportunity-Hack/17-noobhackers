...
/**
 * Chapter payments — dues collection with summary cards, payment table,
 * and full Stripe/PayPal payment flow.
...
function PaymentsContent() {
  ...
  const fetchPayments = useCallback(async () => {
    try {
      ...
      const res = await fetch(`${API_URL}payments?chapterId=${info.chapterId}`, {
  ...
  const totalPaid = payments.filter(p => p.status === "succeeded").reduce((s, p) => s + Number(p.totalAmount), 0);
  const totalStudents = payments.filter(p => p.dueType === "student_enrollment").reduce((s, p) => s + Number(p.quantity), 0);
  const totalCoaches = payments.filter(p => p.dueType === "coach_certification").reduce((s, p) => s + Number(p.quantity), 0);
    ...
    try {
      ...
      if (isRowPayment && payingRowId) {
        ...
        res = await fetch(`${API_URL}payments`, {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: `Bearer ${user?.idToken}` },
          body: JSON.stringify({ chapterId, paymentMethod: method, dueType, quantity, payerEmail: user?.email }),
        });
      }
      const data = await res.json();
      if (res.ok) {
        await fetchPayments();
        setStep("dashboard"); setQuantity(1); setPayingRowId(null);
  ...
  if (step === "stripe_checkout") {
    return (
      <StripeCheckout
        dueType={dueType} quantity={quantity} chapterId={chapterId}
        onSuccess={async () => {
          if (isRowPayment && payingRowId) {
            try {
          ...
          await fetchPayments(); setStep("dashboard"); setPayingRowId(null);
        }}
  ...
  if (step === "pay") {
    return (
      <div className="mx-auto max-w-lg px-4 py-10">
        ...
        <h1 className="text-2xl font-bold text-wial-gray-900">Pay {chapterName} Dues</h1>
        ...
        <div className="mt-6 space-y-4">
          ...
          <label className="block">
            <span className="text-sm font-medium text-wial-gray-700">Number of {dueType === "student_enrollment" ? "students" : "coaches"}</span>
            <input type="number" min={1} max={1000} value={quantity}
              onChange={(e) => { if (!isRowPayment) setQuantity(Math.max(1, parseInt(e.target.value) || 1)); }}
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setStep("pay")} className="rounded-lg bg-wial-red px-5 py-2.5 text-sm font-bold text-white shadow-md shadow-wial-red/20 hover:bg-wial-red-light hover:shadow-lg">
          + New Payment
        </button>
      </div>

      <div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
        <div className="rounded-xl border border-wial-gray-200 bg-white p-5">
          <p className="text-sm text-wial-gray-500">Total Paid</p>
          <p className="mt-1 text-3xl font-bold text-wial-success">${totalPaid.toLocaleString()}</p>
        </div>
        <div className="rounded-xl border border-wial-gray-200 bg-white p-5">
          <p className="text-sm text-wial-gray-500">Students Enrolled</p>
          <p className="mt-1 text-3xl font-bold text-wial-gray-900">{totalStudents}</p>
        </div>
        <div className="rounded-xl border border-wial-gray-200 bg-white p-5">
          <p className="text-sm text-wial-gray-500">Coaches Certified</p>
          <p className="mt-1 text-3xl font-bold text-wial-gray-900">{totalCoaches}</p>
        </div>
        <div className="rounded-xl border border-wial-gray-200 bg-white p-5">
          <p className="text-sm text-wial-gray-500">Transactions</p>
          <p className="mt-1 text-3xl font-bold text-wial-gray-900">{payments.length}</p>
        </div>
      </div>

      {payments.length === 0 ? (
        <div className="mt-10 text-center py-12 rounded-xl border border-dashed border-wial-gray-300">
          <p className="text-wial-gray-400">No payments yet. Click "+ New Payment" to pay chapter dues.</p>
        </div>
      ) : (
        <div className="mt-8 overflow-hidden rounded-xl border border-wial-gray-200 shadow-sm">
          <table className="w-full text-left text-sm">
            <thead className="bg-wial-gray-50">
              <tr>
                <th className="px-4 py-3 font-medium text-wial-gray-500">#</th>
                <th className="px-4 py-3 font-medium text-wial-gray-500">Type</th>
                <th className="px-4 py-3 font-medium text-wial-gray-500">Qty</th>
                <th className="px-4 py-3 font-medium text-wial-gray-500">Amount</th>
                <th className="px-4 py-3 font-medium text-wial-gray-500">Status</th>
                <th className="px-4 py-3 font-medium text-wial-gray-500">Date</th>
                <th className="px-4 py-3 font-medium text-wial-gray-500">Action</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-wial-gray-100">
              {payments.map((p, idx) => {
                ...
                return (
                  <tr key={p.paymentId} className="hover:bg-wial-gray-50 transition-colors">
                    <td className="px-4 py-3 text-wial-gray-400">{idx + 1}</td>
                    <td className="px-4 py-3 text-wial-gray-900">
                      {p.dueType === "student_enrollment" ? "🎓 Student Enrollment" : "🏅 Coach Certification"}
                    </td>
                    <td className="px-4 py-3 text-wial-gray-600">{p.quantity}</td>
                    <td className="px-4 py-3 font-semibold text-wial-gray-900">${p.totalAmount}</td>
                    <td className="px-4 py-3">
                      <span className={`rounded-full px-2.5 py-0.5 text-xs font-bold ${
                        isPaid ? "bg-wial-success/10 text-wial-success" :
                        p.status === "overdue" ? "bg-wial-gold/20 text-wial-gold-dark" :
                        p.status === "failed" ? "bg-wial-error/10 text-wial-error" :
                        "bg-wial-info/10 text-wial-info"
                      }`}>{isPaid ? "paid" : p.status}</span>
                    </td>
                    <td className="px-4 py-3 text-wial-gray-500">{new Date(p.createdAt).toLocaleDateString()}</td>
                    <td className="px-4 py-3">
                      {isPaid ? (
                        <span className="text-sm text-wial-success">✓ Paid</span>
                      ) : p.status === "failed" ? (
                        <span className="text-sm text-wial-error">✗ Failed</span>
                      ) : (
                        <button onClick={() => payRow(p)}
                          className="rounded-lg bg-wial-red px-3 py-1.5 text-xs font-bold text-white shadow-sm hover:bg-wial-red-light hover:shadow-md transition-all">
                          Pay ${p.totalAmount}
                        </button>
                      )}
                    </td>
